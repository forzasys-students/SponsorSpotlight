...
def register_agent_routes(app, agent_task_manager):
    ...
    def agent_query(file_hash):
        ...
        # Query timeline data from the database instead of loading a JSON file
        timeline_db_path = os.path.join(app.config['RESULTS_FOLDER'], file_hash, 'timeline.db')
        if not os.path.exists(timeline_db_path):
            return jsonify({'error': 'Timeline database not found for this file'}), 404
        
        try:
            with TimelineDatabase(timeline_db_path) as db:
                timeline_stats_data = db.get_timeline_stats()
        except Exception as e:
            return jsonify({'error': f'Failed to query timeline database: {str(e)}'}), 500
        ...
        # Pass the database path so tools can perform detailed queries.
        # The old JSON paths are removed as they are deprecated.
